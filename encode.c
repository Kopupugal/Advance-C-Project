...
{
    // Src Image file
    encInfo->fptr_src_image = fopen(encInfo->src_image_fname, "r"); //fopen returns pointer which is collected in file pointer which is declared in struct 
    // Do Error handling
...
/* Read and Validate cla for encodings */ 
Status read_and_validate_encode_args(char *argv[], EncodeInfo *encInfo)
{
    if (strcmp( (strstr(argv[2], ".")), ".bmp") == 0 ) //beautiful.bmp
     {
...
Status do_encoding(EncodeInfo *encInfo)
{
	...
	if(check_capacity(encInfo) == e_success)
	{
		...
		if(encode_magic_string(MAGIC_STRING,encInfo) == e_success)
		{
			...
			{
			    printf("Failed to encode secret file exten\n");
			    return e_failure;
			}
		   }
		    else
	...
	{
	    printf("Secret data can't fit in given image file\n");
	    return e_failure;
	}
    }
    else
